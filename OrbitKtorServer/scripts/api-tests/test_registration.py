...
# ---- izmena kapaciteta
s, current = call("GET", f"/events/{EID}", token=jelena)
s, t = call("PUT", f"/events/{EID}", {**current, "capacity": 1}, token=jelena)
check("organiser lowers capacity below registered -> 409", s == 409, (s, t))
s, e = call("PUT", f"/events/{EID}", {**current, "capacity": 3}, token=jelena)
...
for th in threads:
    th.join()
s, e = call("GET", f"/events/{LAST}", token=jelena)
...
s, t = call("DELETE", f"/events/{started['id']}", token=jelena)
...
s, _ = call("DELETE", f"/events/{EID}", token=jelena)
